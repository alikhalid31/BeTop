...
def create_test_infos_from_protos(raw_data_path, output_path, num_workers=16): 
    ...
    test_infos = get_infos_from_protos(
        data_path=os.path.join(raw_data_path, 'testing_interactive'),
        output_path=os.path.join(output_path, 'processed_scenarios_testing_interactive'),
        num_workers=num_workers
    )
    test_filename = os.path.join(output_path, 'processed_scenarios_test_inter_infos.pkl')
    with open(test_filename, 'wb') as f:
        pickle.dump(test_infos, f)
    print('----------------Waymo info test inter file is saved to %s----------------' % test_filename)
