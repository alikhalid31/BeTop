...
from betopnet.datasets.dataset import DatasetTemplate
from betopnet.utils import common_utils
from betopnet.config import cfg

from copy import deepcopy

class BeTopWaymoDataset(DatasetTemplate):
    def __init__(self, dataset_cfg, training=True, 
        logger=None, inter_pred=False, test=False):
        super().__init__(dataset_cfg=dataset_cfg, training=training,
             logger=logger, inter_pred=inter_pred, test=test)
    ...
    def generate_joint_prediction(self, cur_scene_pred_list, submission):
        assert len(cur_scene_pred_list) == 2, 'cannot exceed two agent for joint prediction!'

        #[k, t, 2]
        src_traj = deepcopy(cur_scene_pred_list[0]['pred_trajs'])
        tgt_traj = deepcopy(cur_scene_pred_list[1]['pred_trajs'])
        src_score = deepcopy(cur_scene_pred_list[0]['pred_scores'])
        tgt_score = deepcopy(cur_scene_pred_list[1]['pred_scores'])

        if submission:
            src_idx = src_score.argsort(descending=True)
            tgt_idx = tgt_score.argsort(descending=True)
            src_score = src_score[src_idx]
            tgt_score = tgt_score[tgt_idx]
            src_traj = src_traj[src_idx]
            tgt_traj = tgt_traj[tgt_idx]
            src_score = src_score + src_score.argsort().argsort()
            tgt_score = tgt_score + tgt_score.argsort().argsort()

        zero_traj = torch.zeros_like(src_traj)
        k, t, d = zero_traj.shape
        #[2, k, t, 2]
        full_src = torch.stack([src_traj, zero_traj], dim=0)
        full_tgt = torch.stack([zero_traj, tgt_traj], dim=0)
        #[2, k, k, t, 2]
        comb_traj = full_src[:, :, None, :, :] + full_tgt[:, None, :, :, :]
        comb_traj = comb_traj.reshape(2, k*k, t, d)

        full_score = src_score[:, None] * tgt_score[None, :]
        full_score = full_score.reshape(k*k)
        top_score, top_inds = full_score.topk(k=6)

        top_traj = comb_traj[:, top_inds, :, :]

        top_traj = top_traj.cpu().numpy()
        top_score = top_score.cpu().numpy()

        if submission:
            sort_idxs = top_score.argsort()[::-1]
            top_score = top_score[sort_idxs]
            top_traj = top_traj[:, sort_idxs]
            top_score = top_score + top_score.argsort().argsort()

        cur_scene_pred_list[0]['pred_trajs'] = top_traj[0]
        cur_scene_pred_list[1]['pred_trajs'] = top_traj[1]

        cur_scene_pred_list[0]['pred_scores'] = top_score
        cur_scene_pred_list[1]['pred_scores'] = top_score
        return cur_scene_pred_list

    def generate_prediction_dicts(self, batch_dict, output_path=None, joint_pred=False, submission=False,
        test=False):
        """
        ...
        for bs_idx in range(batch_dict['batch_size']):
            ...
            for obj_idx in range(start_obj_idx, start_obj_idx + batch_sample_count[bs_idx]):
                single_pred_dict = {
                    ...
                    'pred_trajs': pred_trajs_world[obj_idx, :, :, 0:2].cpu().numpy() if not joint_pred else pred_trajs_world[obj_idx, :, :, 0:2],
                ...
                if not submission:
                    single_pred_dict['gt_trajs'] = input_dict['center_gt_trajs_src'][obj_idx].cpu().numpy()
                
                if submission and (not joint_pred):
                    sort_idxs = single_pred_dict['pred_scores'].argsort()[::-1]
                    score = deepcopy(single_pred_dict['pred_scores'][sort_idxs])
                    score = score + score.argsort().argsort()
                    single_pred_dict['pred_scores'] = score[:6]
                    traj = deepcopy(single_pred_dict['pred_trajs'][sort_idxs])
                    single_pred_dict['pred_trajs'] = traj[:6]

                cur_scene_pred_list.append(single_pred_dict)
            if joint_pred:
                cur_scene_pred_list = self.generate_joint_prediction(cur_scene_pred_list, submission)
            
            if output_path is not None:
  
                save_name = f"/{input_dict['scenario_id'][obj_idx]}.pkl"
                with open(output_path+save_name, 'wb') as writer:
                    pickle.dump(cur_scene_pred_list, writer)
