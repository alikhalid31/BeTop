...
def transform_preds_to_waymo_format_custom(pred_dicts, top_k_for_eval=-1, eval_second=8, joint_pred=False):
    print(f'Total number for evaluation (intput): {len(pred_dicts)}')
    temp_pred_dicts = []
    for k in range(len(pred_dicts)):
        if isinstance(pred_dicts[k], list):
...
def waymo_evaluation_custom(pred_dicts, top_k=-1, eval_second=8, num_modes_for_eval=6, joint_pred=False):

    pred_score, pred_trajectory, gt_infos, object_type_cnt_dict = transform_preds_to_waymo_format_custom(
        pred_dicts, top_k_for_eval=top_k, eval_second=eval_second, joint_pred=False
    )
...
def transform_preds_to_waymo_format_sliding_window(pred_dicts, top_k_for_eval=-1, eval_second=8, joint_pred=False):
    print(f'Total number for evaluation (intput): {len(pred_dicts)}')
    temp_pred_dicts = []
    for k in range(len(pred_dicts)):
        if isinstance(pred_dicts[k], list):
...
def waymo_evaluation_sliding_window(pred_dicts, top_k=-1, eval_second=8, current_time_stamp=10, num_modes_for_eval=6 ,joint_pred=False):
